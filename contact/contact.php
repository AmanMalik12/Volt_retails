...
<body>
	...
	<script>
		...
		if (!$conn) {
			$popDialouge2 = '<div class="popupMessage" id="popUp">
			<div class="popUp">
				<span>Message not sent 2</span>
				<button class="btn btn-danger" onclick="hidePopup()">Ok</button>
			</div>
		</div>
		...
		<script>
			...
			$sql = "INSERT INTO inquiryProduct (productId, name, mobile, email, query) VALUES (?,?,?,?,?)";
			$smtm = $conn->prepare($sql);
			$smtm->bind_param("issss", $p, $name, $mobile, $email, $message);
